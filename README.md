# QuestionaryApp
Mit dieser kleinen Web-Anwendung wird dem Benutzer ein zuvor definierter Fragebogen angezeigt.   
...
Die Datei für die Definition eines Fragebogens ist eine JSON-Datei,
die beim Upload über das Frontend direkt zum Backend weitergereicht und erst dort geparst wird.
...
zeigen, die den Inhalt einer solche Definitionsdatei beschreibt (beginnend mit dem äußersten Objekt):
    ...
    type QuestionaryDef = {
        ...
        questions: QuestionDef[]        // Definitionen der einzelnen Fragen
    }
    ...
    interface QuestionDef_Base {
        id: string                      // vom Ersteller der Datei festgelegte ID
                                        //    der Frage (Anmerkungen: siehe unten)
        text: string                    // der Text der eigentlichen Frage
        conditions: ConditionsGroupDef  // Definition von Bedingungen für diese
                                        //    Frage (Anmerkungen: siehe unten)
    }
    ...
    interface ChoiceQuestionDef extends QuestionDef_Base {
        type: "SINGLE" | "MULTIPLE",    // soll nur eine Option wählbar oder
                                        //    Mehrfach-Auswahl möglich sein
        options: OptionDef[],           // welche Optionen stehen zur Auswahl
    }

    interface QuestionGroupDef extends QuestionDef_Base {
        type: "GROUP",                  // eine Frage, die aus mehreren Teil-Fragen
                                        //    besteht; darf jedoch keine weitere
                                        //    QuestionGroup enthalten
    ...
    type OptionDef = {
        ...
        label?: string                  // Beschriftung dieser Option;
                                        //    kann auch entfallen
                                        //    -> dann wird der Text von 'value' als
                                        //    Beschriftung verwendet
    }


    type ConditionsGroupDef = {
        type: "ALL" | "ONE"             // müssen alle Teilbedingungen erfüllt
                                        //    sein oder reicht eine ?
        conditions: ConditionDef[]      // Liste der Teilbedingungen
    }

    type ConditionDef = {
        question_id: string             // ID der Frage, auf die sich die Bedingung
                                        //    bezieht (Anmerkungen: siehe unten)
        value: PolymorphicValue         // Wert, mit dem diese Frage (-> question_id )
                                        //    beantwortet worden sein muss, damit
                                        //    die Bedingung als erfüllt gilt
    }
    ...
    type PolymorphicValue =
        ...
        |	{
                ...
                value: string,          // entspricht dem, was in OptionDef als
                                        //    'value' festgelegt wurde
...
als Ganzes keine Antworten haben kann, sondern nur ihre Teilfragen, auf die sich dann wieder bezogen werden kann.   
In `ConditionDef.value` wird der gesuchte Wert als `string` oder als `boolean` abgelegt,
gekapselt als Objekt und mit `type` zu Unterscheidung.
...
* eine Fragebogen-Definition ohne eine einzige Frage (-> `QuestionaryDef.questions`)
...
* eie ID (`QuestionDef_Base.id`) einer Frage darf nicht leer sein oder nur aus Leerzeichen bestehen
...
* ein geforderter Wert (`ConditionDef.value`) in einer Bedingung, der in der entsprechenden Frage gar nicht vorkommt (`ChoiceQuestionDef.options` bzw. `OptionDef.value`)
* ein geforderter Wert, dessen Typ nicht zu Frage passt (z.B. `"BOOL"` in Bedingung für eine `ChoiceQuestion`)
...
ich auch hier die Type-Definition aus TypeScript:
    ...
    interface Answer_Base {
        questionId: string                        // ID der Frage, von der die
                                                  //    Antwort(en) stamm(t/en)
    }
    ...
    interface SingleChoiceAnswer extends Answer_Base {
        ...
        answer: string                            // entspricht dem Wert aus
                                                  //    OptionDef.value aus Definition
    }

    interface MultipleChoiceAnswer extends Answer_Base {
        type: "MULTIPLE"                          // Antworten einer Mehrfach-Auswahl
        answers: string[]                         // enthält Werte aus mehreren
                                                  //    OptionDef.value aus Definition
    }

    type Answer =                                 // eine Antwort kann einem der 3
        | BoolAnswer                              //    obigen Typen entsprechen;
        | SingleChoiceAnswer                      //    'Answer' entspricht der
        | MultipleChoiceAnswer                    //    Antwort / den Antworten
                                                  //    einer Frage

    type Download = {
        sessionId: string                         // Session ID, anhand der der
                                                  //    Anwender während der Befragung
                                                  //    identifiziert wird
        answers: Answer[]                         // List der Antworten; zu jeder
                                                  //    beantworteten Frage ein Eintrag
    }
